...
<div class="wrapper col2">
  <div id="header" class="clear">
	...
	if($username && $password && $residence && $department && $course && $name)
	{
		...
		if($ngapi==0)
		{
			$insertstudentdetails=mysqli_query($con,"insert into student_details (year,name,residence,course,idnumber,password,department,clearance_status) values ('$year','$name','$residence','$course','$username','$password','$department','2')");
			if($insertstudentdetails)
			{
				$insertict=mysqli_query($con,"insert into department_ict (idnumber,clearance_status,basket) values ('$username','2','')");
				if($insertict)
				{
